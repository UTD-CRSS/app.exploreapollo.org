...
async function getTranscriptsTrs(url){
    ...
    for (var i=0; i< transcriberContent.length; i+=2){
        ...
        if (i / 2 > 0){
            transcripts[(i / 2 ) - 1]['endTime'] = transcriberContent[i]['attributes']['time']
...
export default async function getTranscripts(url) {
    ...
    if (url.substring(url.length- 4) === "json"){
        await getTranscriptsJson(url)
        .then(response => transcripts = response)
        return transcripts
    }else if (url.substring(url.length- 3) === "trs"){
        await getTranscriptsTrs(url)
        .then(response => transcripts = response)
        return transcripts
    }else return []
