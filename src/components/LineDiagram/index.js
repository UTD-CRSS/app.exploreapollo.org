...
export class LineDiagram extends Component {
  ...
  render() {
    ...
    if (
      data.series.every((datum) => {
        return datum.value.size < 1;
      })
    ) {
    ...
    const diagramData = rawAll.map((datum) => {
      return {
        data: datum.value.map((datum) => {
          return Number(datum.data);
        }),
      };
    });
    ...
    return (
      <div style={{ fontFamily: "sans-serif", fontSize: "0.8em" }}>
        <Chart
          ...
          scaleY={{ paddingTop: 5 }}
        >
          <Layer width="90%" height="90%" position="center">
            <Animate ease="linear" duration={1}>
              <Ticks
                axis="y"
                opacity={1.0}
                position="left"
              ...
              <Ticks
                ...
                position="bottom"
                tickVisible={({ tick }) =>
                  tick.x >= tickCurrent && tick.x < tickCurrent + tickDistance
                }
        ...
        <div
          style={{ position: "center", fontSize: "1.2em", textAlign: "center" }}
        >
